...
namespace RecipeScraper
{
    public partial class frmScraper : Form
    {
        ...
        public void DocumentCompletedJson(object sender, System.Windows.Forms.WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            switch (cboScrapMode.SelectedIndex)
            {
                case 1:
                    ...
                    StringReader sr = new StringReader(documentAsIHtmlDocument3.documentElement.outerHTML);
                    document.Load(sr);
                    HtmlNode[] nodes = document.DocumentNode.SelectNodes("/html/body/div[1]/script[1]").ToArray();
                  //  scrapJsonData(nodes);
                    break;
       ...
       /* private void scrapJsonData(HtmlNode[] nodes)
        {
            string  jsonData = "";
        ...
        private bool isRecipeIndex(HtmlAgilityPack.HtmlDocument page)
        {
            bool resultado = false;
            if (page.DocumentNode.SelectNodes(".//a/@href") != null && page.DocumentNode.SelectNodes(".//a/@href").Count > 100)
                resultado = true;
            return resultado;
        }
        ...
        private void btnIniciar_Click(object sender, EventArgs e)
        {
            ...
            if(txtTotalRegistros.Text != "" ) {
                cantidad = Convert.ToInt32(txtTotalRegistros.Text);
                if (cboScrapMode.SelectedIndex == 0)
                {
        ...
        private void getJsonLdDataWeb(string Url)
        {
            baseUrl = Url;
               
            browser.DocumentCompleted += new WebBrowserDocumentCompletedEventHandler(DocumentCompletedJson);
            browser.ScriptErrorsSuppressed = true;
            browser.Visible = true;
            browser.Navigate(Url);
            progressBarBusqueda.Maximum = Convert.ToInt32(txtTotalRegistros.Text);
